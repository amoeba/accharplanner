...
export default {
  ...
  totalXPInvested: (state: State, getters: any) => {
    ...
    getters.specializedSkills.forEach(function(s: string) {
      cost +=
        COST_SKILL_SPECIALIZED[state.build.character.skills[s].invested];
    });
    ...
    if ((
      ...
      cost -= AUGMENTATION_COST[Augmentation.jack_of_all_trades][state.build.character.augmentations.jack_of_all_trades.invested];
    } else if ((
      state.build.character.race === Race.Empyrean) &&
      state.build.character.augmentations.infused_life_magic.invested == 1) {
      cost -= AUGMENTATION_COST[Augmentation.infused_life_magic][state.build.character.augmentations.infused_life_magic.invested];
    } else if ((
      ...
      cost -= AUGMENTATION_COST[Augmentation.eye_of_the_remorseless][state.build.character.augmentations.eye_of_the_remorseless.invested];
    } else if ((
      state.build.character.race === Race.Lugian) &&
      state.build.character.augmentations.might_of_the_seventh_mule.invested == 1) {
      cost -= AUGMENTATION_COST[Augmentation.might_of_the_seventh_mule][state.build.character.augmentations.might_of_the_seventh_mule.invested];
    } else if ((
      state.build.character.race === Race.Tumerok) &&
      state.build.character.augmentations.hand_of_the_remorseless.invested == 1) {
      cost -= AUGMENTATION_COST[Augmentation.hand_of_the_remorseless][state.build.character.augmentations.hand_of_the_remorseless.invested];
    }
  ...
  totalLuminanceXPSpent: (state: State) => {
    ...
    Object.keys(LUMINANCE_AURA_COST).forEach(aura => {
      cost += LUMINANCE_AURA_COST[aura][state.build.character.luminance_auras[aura].invested];
    });
  ...
  strengthBase: (state: State) => {
    return (
      ...
      state.build.character.augmentations.reinforcement_of_the_lugians.invested * 5
    );
  },
  ...
  healthBase: (state: State, getters: any) => {
    const benediction_bonus =
      state.build.character.augmentations.asherons_lesser_benediction.invested ===
        1 || state.build.character.augmentations.asherons_benediction.invested === 1
        ? 1.1
        : 1;
    ...
    return (
      (Math.round(getters.enduranceBase / 2) +
        ...
        cantripBonus(state.build.character.vitals.health.cantrip)) * benediction_bonus
    );
  },
  ...
  alchemyBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.alchemy.training !== Training.UNUSABLE ? getters.alchemyBase : 0) +
      buffBonus(state.build.character.skills.alchemy.buff) +
      cantripBonus(state.build.character.skills.alchemy.cantrip) +
      Math.round(
        (buffBonus(state.build.character.attributes.coordination.buff) + cantripBonus(state.build.character.attributes.coordination.cantrip) +
          buffBonus(state.build.character.attributes.focus.buff) + cantripBonus(state.build.character.attributes.focus.cantrip)) /
        3
      ) +
      (state.build.character.augmentations.jack_of_all_trades.invested === 1 ? 5 : 0) +
      state.build.character.luminance_auras.craftsman.invested +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.alchemy.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0)
    );
  },
  arcane_loreBase: (state: State, getters: any) => {
    if (state.build.character.skills.arcane_lore.training === Training.UNUSABLE) {
      return 0;
    }
  ...
  arcane_loreBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.arcane_lore.training !== Training.UNUSABLE ? getters.arcane_loreBase : 0) +
      buffBonus(state.build.character.skills.arcane_lore.buff) +
      cantripBonus(state.build.character.skills.arcane_lore.cantrip) +
      Math.round(buffBonus(state.build.character.attributes.focus.buff) + cantripBonus(state.build.character.attributes.focus.cantrip) / 3) +
      (state.build.character.augmentations.jack_of_all_trades.invested === 1 ? 5 : 0) +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.arcane_lore.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0)
    );
  },
  armor_tinkeringBase: (state: State, getters: any) => {
    if (state.build.character.skills.armor_tinkering.training === Training.UNUSABLE) {
      return 0;
    }
  ...
  armor_tinkeringBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.armor_tinkering.training !== Training.UNUSABLE ? getters.armor_tinkeringBase : 0) +
      ...
      Math.round(
        (buffBonus(state.build.character.attributes.endurance.buff) + cantripBonus(state.build.character.attributes.endurance.cantrip) +
          buffBonus(state.build.character.attributes.focus.buff) + cantripBonus(state.build.character.attributes.focus.cantrip)) /
        2
      ) +
      (state.build.character.augmentations.jack_of_all_trades.invested === 1 ? 5 : 0) +
      state.build.character.luminance_auras.craftsman.invested +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.armor_tinkering.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0)
    );
  },
  assess_creatureBase: (state: State) => {
    if (state.build.character.skills.assess_creature.training === Training.UNUSABLE) {
      return 0;
    }
  ...
  assess_creatureBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.assess_creature.training !== Training.UNUSABLE ? getters.assess_creatureBase : 0) +
      ...
      (state.build.character.augmentations.jack_of_all_trades.invested === 1 ? 5 : 0) +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.assess_creature.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0)
    );
  },
  assess_personBase: (state: State) => {
    if (state.build.character.skills.assess_person.training === Training.UNUSABLE) {
      return 0;
    }
  ...
  assess_personBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.assess_person.training !== Training.UNUSABLE ? getters.assess_personBase : 0) +
      buffBonus(state.build.character.skills.assess_person.buff) +
      cantripBonus(state.build.character.skills.assess_person.cantrip) +
      (state.build.character.augmentations.jack_of_all_trades.invested === 1 ? 5 : 0) +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.assess_person.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0)
    );
  },
  ...
  cookingBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.cooking.training !== Training.UNUSABLE ? getters.cookingBase : 0) +
      buffBonus(state.build.character.skills.cooking.buff) +
      cantripBonus(state.build.character.skills.cooking.cantrip) +
      (state.build.character.augmentations.jack_of_all_trades.invested === 1 ? 5 : 0) +
      state.build.character.luminance_auras.craftsman.invested +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.cooking.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0) +
      Math.round(
        (buffBonus(state.build.character.attributes.coordination.buff) + cantripBonus(state.build.character.attributes.coordination.cantrip) +
          buffBonus(state.build.character.attributes.focus.buff) + cantripBonus(state.build.character.attributes.focus.cantrip)) /
        3
  ...
  creature_enchantmentBase: (state: State, getters: any) => {
    if (state.build.character.skills.creature_enchantment.training === Training.UNUSABLE) {
      return 0;
    }
  ...
  creature_enchantmentBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.creature_enchantment.training !== Training.UNUSABLE ? getters.creature_enchantmentBase : 0) +
      buffBonus(state.build.character.skills.creature_enchantment.buff) +
      cantripBonus(
        state.build.character.skills.creature_enchantment.cantrip
      ) +
      Math.round(
        (buffBonus(state.build.character.attributes.focus.buff) + cantripBonus(state.build.character.attributes.focus.cantrip) +
          buffBonus(state.build.character.attributes.self.buff) + cantripBonus(state.build.character.attributes.self.cantrip)) /
        4
      ) +
      (state.build.character.augmentations.jack_of_all_trades.invested === 1 ? 5 : 0) +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.creature_enchantment.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0)
    );
  },
  ...
  deceptionBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.deception.training !== Training.UNUSABLE ? getters.deceptionBase : 0) +
      buffBonus(state.build.character.skills.deception.buff) +
      cantripBonus(state.build.character.skills.deception.cantrip) +
      (state.build.character.augmentations.jack_of_all_trades.invested === 1 ? 5 : 0) +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.deception.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0)
    );
  },
  dirty_fightingBase: (state: State, getters: any) => {
    if (state.build.character.skills.dirty_fighting.training === Training.UNUSABLE) {
      return 0;
    }
  ...
  dirty_fightingBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.dirty_fighting.training !== Training.UNUSABLE ? getters.dirty_fightingBase : 0) +
      ...
      Math.round(
        (buffBonus(state.build.character.attributes.strength.buff) + cantripBonus(state.build.character.attributes.strength.cantrip) +
          buffBonus(state.build.character.attributes.coordination.buff) + cantripBonus(state.build.character.attributes.coordination.cantrip)) /
        3
      ) +
      (state.build.character.augmentations.jack_of_all_trades.invested === 1 ? 5 : 0) +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.dirty_fighting.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0)
    );
  },
  dual_wieldBase: (state: State, getters: any) => {
    if (state.build.character.skills.dual_wield.training === Training.UNUSABLE) {
      return 0;
    }
  ...
  dual_wieldBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.dual_wield.training !== Training.UNUSABLE ? getters.dual_wieldBase : 0) +
      ...
      (state.build.character.augmentations.jack_of_all_trades.invested === 1 ? 5 : 0) +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.dual_wield.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0)
    );
  },
  finesse_weaponsBase: (state: State, getters: any) => {
    if (state.build.character.skills.finesse_weapons.training === Training.UNUSABLE) {
      return 0;
    }
  ...
  finesse_weaponsBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.finesse_weapons.training !== Training.UNUSABLE ? getters.finesse_weaponsBase : 0) +
      ...
      Math.round(
        (buffBonus(state.build.character.attributes.coordination.buff) + cantripBonus(state.build.character.attributes.coordination.cantrip) +
          buffBonus(state.build.character.attributes.quickness.buff) + cantripBonus(state.build.character.attributes.quickness.cantrip)) /
        3
      ) +
      (state.build.character.augmentations.jack_of_all_trades.invested === 1 ? 5 : 0) +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.finesse_weapons.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0)
    );
  },
  ...
  fletchingBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.fletching.training !== Training.UNUSABLE ? getters.fletchingBase : 0) +
      ...
      state.build.character.luminance_auras.craftsman.invested +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.fletching.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0)
    );
  },
  ...
  healingBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.healing.training !== Training.UNUSABLE ? getters.healingBase : 0) +
      ...
      (state.build.character.augmentations.jack_of_all_trades.invested === 1 ? 5 : 0) +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.healing.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0)
    );
  },
  heavy_weaponsBase: (state: State, getters: any) => {
    if (state.build.character.skills.heavy_weapons.training === Training.UNUSABLE) {
      return 0;
    }
  ...
  heavy_weaponsBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.heavy_weapons.training !== Training.UNUSABLE ? getters.heavy_weaponsBase : 0) +
      ...
      (state.build.character.augmentations.jack_of_all_trades.invested === 1 ? 5 : 0) +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.heavy_weapons.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0)
    );
  },
  item_enchantmentBase: (state: State, getters: any) => {
    if (state.build.character.skills.item_enchantment.training === Training.UNUSABLE) {
      return 0;
    }
  ...
  item_enchantmentBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.item_enchantment.training !== Training.UNUSABLE ? getters.item_enchantmentBase : 0) +
      ...
      (state.build.character.augmentations.jack_of_all_trades.invested === 1 ? 5 : 0) +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.item_enchantment.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0)
    );
  },
  item_tinkeringBase: (state: State, getters: any) => {
    if (state.build.character.skills.item_tinkering.training === Training.UNUSABLE) {
      return 0;
    }
  ...
  item_tinkeringBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.item_tinkering.training !== Training.UNUSABLE ? getters.item_tinkeringBase : 0) +
      ...
      state.build.character.luminance_auras.craftsman.invested +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.item_tinkering.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0)
    );
  },
  ...
  jumpBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.jump.training !== Training.UNUSABLE ? getters.jumpBase : 0) +
      ...
      (state.build.character.augmentations.jack_of_all_trades.invested === 1 ? 5 : 0) +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.jump.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0)
    );
  },
  leadershipBase: (state: State) => {
    if (state.build.character.skills.leadership.training === Training.UNUSABLE) {
      return 0;
    }
  ...
  leadershipBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.leadership.training !== Training.UNUSABLE ? getters.leadershipBase : 0) +
      buffBonus(state.build.character.skills.leadership.buff) +
      cantripBonus(state.build.character.skills.leadership.cantrip) +
      (state.build.character.augmentations.jack_of_all_trades.invested === 1 ? 5 : 0) +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.leadership.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0)
    );
  },
  life_magicBase: (state: State, getters: any) => {
    if (state.build.character.skills.life_magic.training === Training.UNUSABLE) {
      return 0;
    }
  ...
  life_magicBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.life_magic.training !== Training.UNUSABLE ? getters.life_magicBase : 0) +
      ...
      (state.build.character.augmentations.jack_of_all_trades.invested === 1 ? 5 : 0) +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.life_magic.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0)
    );
  },
  light_weaponsBase: (state: State, getters: any) => {
    if (state.build.character.skills.light_weapons.training === Training.UNUSABLE) {
      return 0;
    }
  ...
  light_weaponsBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.light_weapons.training !== Training.UNUSABLE ? getters.light_weaponsBase : 0) +
      ...
      (state.build.character.augmentations.jack_of_all_trades.invested === 1 ? 5 : 0) +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.light_weapons.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0)
    );
  },
  ...
  lockpickBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.lockpick.training !== Training.UNUSABLE ? getters.lockpickBase : 0) +
      ...
      state.build.character.luminance_auras.craftsman.invested +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.lockpick.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0)
    );
  },
  ...
  loyaltyBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.loyalty.training !== Training.UNUSABLE ? getters.loyaltyBase : 0) +
      buffBonus(state.build.character.skills.loyalty.buff) +
      cantripBonus(state.build.character.skills.loyalty.cantrip) +
      (state.build.character.augmentations.jack_of_all_trades.invested === 1 ? 5 : 0) +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.loyalty.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0)
    );
  },
  magic_defenseBase: (state: State, getters: any) => {
    if (state.build.character.skills.magic_defense.training === Training.UNUSABLE) {
      return 0;
    }
  ...
  magic_defenseBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.magic_defense.training !== Training.UNUSABLE ? getters.magic_defenseBase : 0) +
      ...
      (state.build.character.augmentations.jack_of_all_trades.invested === 1 ? 5 : 0) +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.magic_defense.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0)
    );
  },
  magic_item_tinkeringBase: (state: State, getters: any) => {
    if (state.build.character.skills.magic_item_tinkering.training === Training.UNUSABLE) {
      return 0;
    }
  ...
  magic_item_tinkeringBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.magic_item_tinkering.training !== Training.UNUSABLE ? getters.alchemyBase : 0) +
      buffBonus(state.build.character.skills.magic_item_tinkering.buff) +
      cantripBonus(
        state.build.character.skills.magic_item_tinkering.cantrip
      ) +
      buffBonus(state.build.character.attributes.focus.buff) + cantripBonus(state.build.character.attributes.focus.cantrip) +
      (state.build.character.augmentations.jack_of_all_trades.invested === 1 ? 5 : 0) +
      state.build.character.luminance_auras.craftsman.invested +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.magic_item_tinkering.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0)
    );
  },
  mana_conversionBase: (state: State, getters: any) => {
    if (state.build.character.skills.mana_conversion.training === Training.UNUSABLE) {
      return 0;
    }
  ...
  mana_conversionBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.mana_conversion.training !== Training.UNUSABLE ? getters.mana_conversionBase : 0) +
      ...
      (state.build.character.augmentations.jack_of_all_trades.invested === 1 ? 5 : 0) +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.mana_conversion.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0)
    );
  },
  melee_defenseBase: (state: State, getters: any) => {
    if (state.build.character.skills.melee_defense.training === Training.UNUSABLE) {
      return 0;
    }
  ...
  melee_defenseBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.melee_defense.training !== Training.UNUSABLE ? getters.melee_defenseBase : 0) +
      ...
      (state.build.character.augmentations.jack_of_all_trades.invested === 1 ? 5 : 0) +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.melee_defense.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0)
    );
  },
  missile_defenseBase: (state: State, getters: any) => {
    if (state.build.character.skills.missile_defense.training === Training.UNUSABLE) {
      return 0;
    }
  ...
  missile_defenseBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.missile_defense.training !== Training.UNUSABLE ? getters.missile_defenseBase : 0) +
      ...
      (state.build.character.augmentations.jack_of_all_trades.invested === 1 ? 5 : 0) +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.missile_defense.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0)
    );
  },
  missile_weaponsBase: (state: State, getters: any) => {
    if (state.build.character.skills.missile_weapons.training === Training.UNUSABLE) {
      return 0;
    }
  ...
  missile_weaponsBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.missile_weapons.training !== Training.UNUSABLE ? getters.missile_weaponsBase : 0) +
      ...
      Math.round(
        buffBonus(state.build.character.attributes.coordination.buff) + cantripBonus(state.build.character.attributes.coordination.cantrip) / 2
      ) +
      (state.build.character.augmentations.jack_of_all_trades.invested === 1 ? 5 : 0) +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.missile_weapons.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0)
    );
  },
  recklessnessBase: (state: State, getters: any) => {
    if (state.build.character.skills.recklessness.training === Training.UNUSABLE) {
      return 0;
    }
  ...
  recklessnessBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.recklessness.training !== Training.UNUSABLE ? getters.recklessnessBase : 0) +
      ...
      (state.build.character.augmentations.jack_of_all_trades.invested === 1 ? 5 : 0) +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.recklessness.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0)
    );
  },
  ...
  runBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.run.training !== Training.UNUSABLE ? getters.runBase : 0) +
      ...
      (state.build.character.augmentations.jack_of_all_trades.invested === 1 ? 5 : 0) +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.run.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0)
    );
  },
  ...
  salvagingBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.salvaging.training !== Training.UNUSABLE ? getters.salvagingBase : 0) +
      buffBonus(state.build.character.skills.salvaging.buff) +
      cantripBonus(state.build.character.skills.salvaging.cantrip) +
      (state.build.character.augmentations.jack_of_all_trades.invested === 1 ? 5 : 0) +
      state.build.character.luminance_auras.craftsman.invested +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.salvaging.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0)
    );
  },
  ...
  shieldBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.shield.training !== Training.UNUSABLE ? getters.shieldBase : 0) +
      ...
      (state.build.character.augmentations.jack_of_all_trades.invested === 1 ? 5 : 0) +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.shield.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0)
    );
  },
  sneak_attackBase: (state: State, getters: any) => {
    if (state.build.character.skills.sneak_attack.training === Training.UNUSABLE) {
      return 0;
    }
  ...
  sneak_attackBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.sneak_attack.training !== Training.UNUSABLE ? getters.sneak_attackBase : 0) +
      ...
      (state.build.character.augmentations.jack_of_all_trades.invested === 1 ? 5 : 0) +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.sneak_attack.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0)
    );
  },
  ...
  summoningBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.summoning.training !== Training.UNUSABLE ? getters.summoningBase : 0) +
      ...
      (state.build.character.augmentations.jack_of_all_trades.invested === 1 ? 5 : 0) +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.summoning.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0)
    );
  },
  two_handed_combatBase: (state: State, getters: any) => {
    if (state.build.character.skills.two_handed_combat.training === Training.UNUSABLE) {
      return 0;
    }
  ...
  two_handed_combatBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.two_handed_combat.training !== Training.UNUSABLE ? getters.two_handed_combatBase : 0) +
      ...
      (state.build.character.augmentations.jack_of_all_trades.invested === 1 ? 5 : 0) +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.two_handed_combat.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0)
    );
  },
  void_magicBase: (state: State, getters: any) => {
    if (state.build.character.skills.void_magic.training === Training.UNUSABLE) {
      return 0;
    }
  ...
  void_magicBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.void_magic.training !== Training.UNUSABLE ? getters.void_magicBase : 0) +
      ...
      (state.build.character.augmentations.jack_of_all_trades.invested === 1 ? 5 : 0) +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.void_magic.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0)
    );
  },
  ...
  war_magicBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.war_magic.training !== Training.UNUSABLE ? getters.war_magicBase : 0) +
      ...
      (state.build.character.augmentations.jack_of_all_trades.invested === 1 ? 5 : 0) +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.war_magic.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0)
    );
  },
  weapon_tinkeringBase: (state: State, getters: any) => {
    if (state.build.character.skills.weapon_tinkering.training === Training.UNUSABLE) {
      return 0;
    }
  ...
  weapon_tinkeringBuffed: (state: State, getters: any) => {
    return (
      (state.build.character.skills.weapon_tinkering.training !== Training.UNUSABLE ? getters.weapon_tinkeringBase : 0) +
      ...
      state.build.character.luminance_auras.craftsman.invested +
      state.build.character.luminance_auras.world.invested +
      (state.build.character.skills.weapon_tinkering.training === Training.SPECIALIZED ? state.build.character.luminance_auras.specialization.invested * 2 : 0)
    );
  },
  ...
  untrainedSkills: (state: State) => {
    return Object.keys(state.build.character.skills).filter(
      key =>
        state.build.character.skills[key].training === Training.UNTRAINED
    );
  },
  unusableSkills: (state: State) => {
    return Object.keys(state.build.character.skills).filter(
      key =>
        state.build.character.skills[key].training === Training.UNUSABLE
    );
  }
};
