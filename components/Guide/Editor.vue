...
const fetchGuide = async function (id: Number): Guide {
  ...
  const { data, error } = await client.from("guides")
    .select().eq("id", id)
...
<template>
  ...
  <form v-if="store.guide" @submit.prevent="createOrUpdateGuide" class="flex flex-col gap-2">
